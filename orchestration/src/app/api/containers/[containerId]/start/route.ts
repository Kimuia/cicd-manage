  ...
  try {
    ...
    if (container.agent_id) {
      ...
      if (agent) {
        ...
        const res = await fetch(`${url}/containers/${containerId}/start`, { method: 'POST' });
        if (!res.ok) {
          const err = await res.json().catch(() => ({ error: { message: 'Unknown error' } }));
          return serverError(err.error?.message ?? 'Failed to start container');
        }
      }
    }

    containerRepository.updateStatus(containerId, 'running');
    eventBus.emit('container:started', { containerId });

    return NextResponse.json({ status: 'started', containerId });
  } catch (err) {
